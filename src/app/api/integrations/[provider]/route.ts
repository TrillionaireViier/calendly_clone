...
export async function GET(
  ...
  switch (provider) {
    case 'google':
      if (!process.env.GOOGLE_CLIENT_ID) return NextResponse.redirect(redirectUri);
      ...
      break;
    case 'zoom':
      if (!process.env.ZOOM_CLIENT_ID) return NextResponse.redirect(redirectUri);
      authUrl = `https://zoom.us/oauth/authorize?response_type=code&client_id=${process.env.ZOOM_CLIENT_ID}&redirect_uri=${redirectUri}`;
      break;
    case 'stripe':
      if (!process.env.STRIPE_CLIENT_ID) return NextResponse.redirect(redirectUri);
      ...
      break;
    case 'salesforce':
      if (!process.env.SALESFORCE_CLIENT_ID) return NextResponse.redirect(redirectUri);
